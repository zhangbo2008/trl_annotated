...
class CPOTrainer(Trainer):
    ...
    def build_tokenized_answer(self, prompt, answer):
        ...
        full_tokenized = self.processing_class(prompt + answer, add_special_tokens=False) # 全编码
        prompt_input_ids = self.processing_class(prompt, add_special_tokens=False)["input_ids"] # 输入的编码
    ...
    @staticmethod
    def concatenated_inputs( # 在dim=0维度上进行concat
    ...
    def _shift_right(self, input_ids):
        ...
        if is_torch_fx_proxy(input_ids):
            ...
            shifted_input_ids = torch.full(input_ids.shape[:-1] + (1,), self.decoder_start_token_id) # 每一个batch开头都天上start_token
            shifted_input_ids = torch.cat([shifted_input_ids, input_ids[..., :-1]], dim=-1)#再拼接input_ids
        else:
