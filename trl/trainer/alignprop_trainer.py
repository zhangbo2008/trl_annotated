...
class AlignPropTrainer(PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        self.prompt_fn = prompt_function #使用这个函数来生成提示词
        self.reward_fn = reward_function # 奖励函数用来打分
        self.config = config
        self.image_samples_callback = image_samples_hook

        accelerator_project_config = ProjectConfiguration(**self.config.project_kwargs)#通用的配置参数.
        ...
        if self.accelerator.is_main_process:
            self.accelerator.init_trackers(#配置log
    ...
    def step(self, epoch: int, global_step: int):
        ...
        self.sd_pipeline.unet.train() # unet部分打开.
        ...
        for _ in range(self.config.train_gradient_accumulation_steps):
            with self.accelerator.accumulate(self.sd_pipeline.unet), self.autocast(), torch.enable_grad():
                ...
                rewards = self.compute_rewards(prompt_image_pairs) #对每一个图片进行打分.
