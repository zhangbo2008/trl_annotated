...
class StringStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        """Returns true if all generated sequences contain any of the stop strings.""" # 包含结束符就返回true
        if self.first_call:
            self.generated_tokens = [1 for _ in range(input_ids.shape[0])] # batchsiez维度上上都设置为1作为句子长度.
        ...
        return all(done)# 表示整个batch都结束了没有新的token生成了.才进行停止.
